...
[README](./style-guide/README.md), which reads as a procedure and links each step.
...
| [`style-guide/02-page-structure.md`](./style-guide/02-page-structure.md) | Document type, section grouping, chunking          |
| [`style-guide/03-elements.md`](./style-guide/03-elements.md)             | Admonitions, code blocks, procedures, tabs, images |
...
Overview and index pages use a single `<ContentListings id="..." />` component for curated link sections. For when to use one, see [the style guide](./style-guide/03-elements.md#content-listings). Refer to [`storage.data.ts`](data/content-listings/storage.data.ts) and [`storage.mdx`](content/guides/storage.mdx) for a full example.
