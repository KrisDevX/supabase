...
The stack you started in [Run a local Supabase project](#running-a-local-supabase-project) runs only on your machine. To move your work to a hosted project, sign in to the CLI and link your local directory to that project:

1. Sign in to the CLI. The command opens a browser to generate an access token:

   ```bash
   supabase login
   ```

2. Link the directory to your hosted project. Find the project ID in your Supabase Dashboard URL, `https://supabase.com/dashboard/project/<project-id>`:

   ```bash
   supabase link --project-ref <project-id>
   ```

Your local directory now targets that project for commands that act on a remote database, such as `db pull` and `db push`.

For the full path from a local stack to a deployed project, including pulling an existing schema and pushing migrations, see [Local development workflow](/docs/guides/local-development/cli-workflows). To run deployments from CI, see [Configure GitHub Actions](/docs/guides/deployment/managing-environments#configure-github-actions).
